(() => {
  ...
  if (root) {
    ...
    const toggle = document.getElementById('cckChatToggle');
    const close = document.getElementById('cckChatClose');
    const body = document.getElementById('cckChatBody');
    const oldForm = document.getElementById('cckChatForm');
    const input = document.getElementById('cckChatInput');
    if (!panel || !body || !oldForm || !input) return;

    // Replace the original demo listeners without changing the existing markup.
    ...
    async function poll() {
      ...
      try {
        ...
        (data.messages || []).forEach(add);
        if (data.status === 'waiting') humanRequested = true;
      } catch (_) {}
    }
    ...
    if (toggle) toggle.addEventListener('click', () => { panel.classList.toggle('open'); toggle.setAttribute('aria-expanded', panel.classList.contains('open') ? 'true' : 'false'); });
    if (close) close.addEventListener('click', () => { panel.classList.remove('open'); toggle && toggle.setAttribute('aria-expanded','false'); });
